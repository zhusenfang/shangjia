...
// var hotlist=[]
var pagelist=[]
export default class SearchShouCang extends Component {
    constructor(props){
        super(props);
        // this.shit().bind(this)
        this.state={
    ...
    componentWillMount(){
        // postFetch(API.SearchHot,null,(result)=>{
        ...
        if(this.state.isStyle==false){
            contview=(<View><View style={styles.his}>
                    ...
                    <View style={comstyle.leftview}>
                        ...
                        <TouchableOpacity onPress={()=>{this.props.navigation.navigate('SearchShouCangHistory')}}>
    ...
    laji(){
        Storage.delete('searsh').then((tags)=>{
            this.setState({
                hisList:''
            })
        })
    ...
    searchdong(){
        ...
        Storage.get('searsh').then((taggs)=>{
            if(taggs==null){
                taggs = new Array();
            }
            taggs.push(hist)
            AsyncStorage.setItem('searsh', JSON.stringify(taggs),);
        })

        postFetch(API.SearchShouCangdet,{keyword:this.state.text},(result)=>{
            // alert(JSON.stringify(result))
            ...
            if(result.status==1){
                ...
                this.setState({
                    dataSource:this.state.dataSource.cloneWithRows(result.data.forumPostCollectList),
    ...
    _renderRow=(rowData,sectionID,rowID)=>{
        return(<TouchableOpacity style={styles.toux} onPress={this.shit.bind(this,rowData.id)}>
            <View style={[styles.item,{justifyContent:'space-between'}]}>
                <View style={comstyle.rightview}>
                    <Image source={{uri:rowData.forumPostDo.userMember&&rowData.forumPostDo.userMember.picUrl}} style={styles.image}/>
                    <View style={styles.chixu}>
                        <Text style={{fontSize:12,color:'#282828'}}>{rowData.forumPostDo.userMember&&rowData.forumPostDo.userMember.nickname}</Text>
                        {/*<Text>{rowData.createTime}</Text>*/}
                        <Text style={{fontSize:10,color:'#B2B2B2'}}>{new Date(rowData.forumPostDo.createTime).getHours()+'小时前'}</Text>
                    </View>
                </View>
            ...
            <View style={{flexDirection:'column'}}>
                <ListView
                    dataSource={this.state.dataSourceimg.cloneWithRows(rowData.forumPostDo.forumThreadResources)}
